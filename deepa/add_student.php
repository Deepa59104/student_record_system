...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(empty($errors)) {
        $q = "INSERT INTO student (full_name, email, teacher_id, course_id, enrolled_date)
              VALUES ('{$old['full_name']}', '{$old['email']}', 1, {$old['course_id']}, '{$old['enrolled_date']}')";
...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Plus Jakarta Sans', sans-serif; background: #0f0a1e; min-height: 100vh; position: relative; overflow-x: hidden; color: white; }
        ...
        .breadcrumb { display: flex; align-items: center; gap: 8px; font-size: 13px; color: rgba(255,255,255,0.3); margin-bottom: 24px; }
        ...
        input, select { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 10px; padding: 11px 14px; color: white; font-size: 13px; font-family: inherit; outline: none; transition: border-color 0.2s; width: 100%; }
        ...
        .btn-add { background: linear-gradient(135deg, #7c3aed, #a855f7); color: white; border: none; padding: 12px 28px; border-radius: 12px; font-size: 14px; font-weight: 600; font-family: inherit; cursor: pointer; box-shadow: 0 4px 18px rgba(124,58,237,0.4); }
...
<body>
    ...
    <div class="content">
        <div class="breadcrumb">
            <a href="student_list.php">← Back to Students</a>
        </div>
        ...
        <?php if(isset($errors['general'])): ?>
            <div class="alert-error-box"><?php echo $errors['general']; ?></div>
        <?php endif; ?>
                ...
                <div class="form-grid">
                        ...
                        <select name="course_id" class="<?php echo isset($errors['course_id'])?'is-error':''; ?>">
                            ...
                            <?php while($c = mysqli_fetch_assoc($courses)): ?>
                                <option value="<?php echo $c['course_id']; ?>"
                                    <?php echo (isset($old['course_id']) && $old['course_id'] == $c['course_id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="enrolled_date"
                               value="<?php echo htmlspecialchars($old['enrolled_date'] ?? ''); ?>"
                               max="<?php echo date('Y-m-d'); ?>"
                               class="<?php echo isset($errors['enrolled_date'])?'is-error':''; ?>">
